...
export default {
  ...
  computed: {
    sortedCanteens() {
      return [...this.canteens].sort((a, b) => this.getOccupancyPercent(a) - this.getOccupancyPercent(b))
    },
  },
  async mounted() {
  ...
  methods: {
    ...
    initMap() {
      const L = window.L
      this.map = L.map('leaflet-map', { center: [1.2966, 103.7764], zoom: 15, zoomControl: true })
    ...
    refreshMarkers() {
      ...
      this.canteens.forEach((c) => {
        ...
        const marker = L.circleMarker(coords, {
          radius: 14, fillColor: color, color: '#fff', weight: 3, opacity: 1, fillOpacity: 0.92,
        })
        const popupHtml = `
        ...
        marker.on('click', () => { this.selectedCanteenId = c.id })
    ...
    async showRoute() {
      ...
      this.activeRoute = null; this.routeError = null; this.routeLoading = true
      try {
        ...
        this.routeLine = L.polyline(latlngs, { color: '#F37021', weight: 5, opacity: 0.9, lineCap: 'round', lineJoin: 'round' }).addTo(this.map)
        this.map.fitBounds(this.routeLine.getBounds(), { padding: [60, 60] })
        this.activeRoute = { start, dest, distanceKm: (route.distance / 1000).toFixed(2), walkMins: Math.round(route.duration / 60) }
      } catch (err) {
        this.routeError = 'Could not load walking route. Please try again.'
        this.routeLine = L.polyline([startCoords, destCoords], { color: '#F37021', weight: 4, opacity: 0.7, dashArray: '10, 8' }).addTo(this.map)
        ...
        this.activeRoute = { start, dest, distanceKm: dist.toFixed(2), walkMins: Math.round((dist / 5) * 60) }
      } finally {
    ...
    clearRoute() {
      if (this.routeLine) { this.routeLine.remove(); this.routeLine = null }
      this.activeRoute = null; this.routeError = null
    ...
    getOccupancyDisplay(c) { return Math.round(this.getOccupancyPercent(c)) },
    getCrowdClass(c) {
...
.map-page { min-height: 100vh; display: flex; flex-direction: column; font-family: Arial, sans-serif; background: #f0f2f5; }
...
.page-body { display: flex; flex: 1; height: calc(100vh - 57px); overflow: hidden; }

.sidebar { width: 340px; flex-shrink: 0; background: white; border-right: 1px solid #e5e7eb; display: flex; flex-direction: column; overflow: hidden; }
.sidebar-inner { padding: 24px 20px; overflow-y: auto; height: 100%; }
...
.route-planner { background: #f8fafc; border-radius: 12px; padding: 16px; margin-bottom: 24px; border: 1px solid #e2e8f0; }
.field-group { display: flex; flex-direction: column; gap: 6px; }
.field-group label { font-size: 0.8rem; font-weight: 700; color: #555; text-transform: uppercase; letter-spacing: 0.04em; }
.field-group select { padding: 9px 12px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 0.9rem; background: white; color: #333; outline: none; cursor: pointer; }
...
.swap-btn { background: white; border: 1px solid #d1d5db; border-radius: 50%; width: 30px; height: 30px; font-size: 1rem; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #555; transition: 0.2s; flex-shrink: 0; }
.swap-btn:hover { background: #f3f4f6; border-color: #0A1C3E; }

.btn-route { width: 100%; margin-top: 14px; background: #F37021; color: white; border: none; padding: 11px; border-radius: 8px; font-weight: 700; font-size: 0.95rem; cursor: pointer; transition: background 0.2s; }
...
.route-card { margin-top: 14px; background: white; border-radius: 10px; padding: 14px; border: 1.5px solid #F37021; }
.route-card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
.route-badge { background: #F37021; color: white; font-size: 0.7rem; font-weight: 700; padding: 2px 10px; border-radius: 20px; letter-spacing: 0.05em; text-transform: uppercase; }
.clear-btn { background: none; border: none; color: #888; font-size: 0.8rem; cursor: pointer; padding: 2px 6px; }
...
.route-connector { width: 2px; height: 18px; background: repeating-linear-gradient(to bottom, #ccc 0, #ccc 4px, transparent 4px, transparent 8px); margin: 4px 0 4px 5px; }
.route-meta { display: flex; gap: 12px; margin-top: 10px; font-size: 0.82rem; color: #666; font-weight: 600; }

.btn-details { width: 100%; margin-top: 12px; background: #0A1C3E; color: white; border: none; padding: 9px; border-radius: 7px; font-size: 0.85rem; font-weight: 700; cursor: pointer; transition: background 0.2s; }
...
.list-item { display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 10px; cursor: pointer; transition: background 0.15s; margin-bottom: 4px; }
...
.map-legend { position: absolute; bottom: 24px; right: 16px; background: white; border-radius: 10px; padding: 10px 14px; display: flex; align-items: center; gap: 14px; box-shadow: 0 2px 12px rgba(0,0,0,0.12); z-index: 500; font-size: 0.82rem; }
