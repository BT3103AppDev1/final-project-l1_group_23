  ...
  <div class="recommended-page">
    ...
    <div class="hero-banner">
      ...
      <div class="ranking-note">
        <strong>Canteens<br />Ranked in descending<br />order according to<br />occupancy level</strong>
        <div class="arrow">↗</div>
      </div>
    </div>
      ...
      <div class="cards-grid">
        ...
        >
          ...
          <div class="img-wrap">
            ...
            />
            <button class="fav-btn" @click.stop="toggleFav(canteen.id)">
              {{ favs.includes(canteen.id) ? '❤️' : '🤍' }}
            </button>
          </div>
...
// ── Computed: sort by occupancy % ascending ──────────────
const sortedCanteens = computed(() =>
  [...canteens.value].sort((a, b) => a.pct - b.pct)
)
    ...
    unsub = onSnapshot(
      ...
      async (snap) => {
        ...
        canteens.value = snap.docs
          ...
          .filter(
            (canteen) =>
              canteen.imageUrl !== '/lion.jpg' &&
              canteen.id !== 'pgp-canteen'
          )
...
.brand-nus { color: #f97316; }
.brand-pulse { color: #fff; }
...
.nav-link.active { background: #f97316; color: #fff; font-weight: 600; }
.nav-link.icon-only { padding: 5px 9px; }
...
.arrow { font-size: 1.4rem; margin-top: 3px; }
...
.left-label { display: flex; align-items: center; gap: 9px; }
.badge-choice {
  background: #fef08a;
  color: #78350f;
  font-size: 0.69rem;
...
.label-bold { font-weight: 700; font-size: 0.93rem; color: #111; }
.right-label { font-size: 0.88rem; color: #555; }
...
.canteen-card.is-top { border: 2px solid #e8601e; }
...
.canteen-card:hover .card-img { transform: scale(1.05); }
...
.crowd-lbl { font-size: 0.74rem; font-weight: 600; }
.crowd-lbl.low { color: #16a34a; }
.crowd-lbl.mid { color: #b45309; }
.crowd-lbl.high { color: #dc2626; }

.capacity { margin-left: auto; font-size: 0.7rem; color: #bbb; }
...
/* ── Status ── */
.status-block { text-align: center; padding: 56px 20px; color: #888; }
...
@media (max-width: 580px) {
  ...
  .ranking-note, .navbar-links { display: none; }
}
</style>